...
# 调用第一个接口
curl --silent --request POST --url "http://${local_ip}:8096/emby/Auth/Keys?App=emby&X-Emby-Token=${x_emby_token}"

# 调用第二个接口，获取响应并提取AccessToken
response=$(curl --silent --request GET --url "http://${local_ip}:8096/emby/Auth/Keys?X-Emby-Token=${x_emby_token}")
